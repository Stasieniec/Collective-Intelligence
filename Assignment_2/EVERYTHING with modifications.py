...
class Foxes(Agent):
    ...
    def reproduction(self):
          # 50% chance to reproduce upon meeting an opposite-sex partner
        if self.eat_flag:
            ...
            if compatible_partner and random.random() < self.reproduction_chance:
                print('Reproduction!')
                self.reproduce()
                self.eat_flag = False
        return
...
class Rabbits(Agent):
    ...
    def reproduction(self):
        # 50% chance to reproduce upon meeting an opposite-sex partner
        if self.gender == 'female' and random.random() < self.p_reproduction:
            ...
            if compatible_partner:
                # Check current rabbit population
                current_rabbit_population = sum(1 for agent in self._simulation._agents if isinstance(agent, Rabbits) and agent.alive)
                num_offspring = 0

                if current_rabbit_population < 5:  # Example threshold, adjust as needed
                    num_offspring = random.randint(2, 4)  # More offspring when population is low
                else:
                    num_offspring = random.randint(1, 2)  # Fewer offspring when population is high
...
class CompetitionSimulation(Simulation):
    ...
    def __init__(self, config):
        ...
        self.initial_rabbit_reproduction_prob = Rabbits.p_reproduction
        self.initial_foxes_reproduction_prob = Foxes.reproduction_chance

    def check_fox_population(self):
        current_fox_population = sum(1 for agent in self._agents if isinstance(agent, Foxes) and agent.alive)
        max_fox_population = 20  # Example threshold, adjust as needed

        if current_fox_population > max_fox_population:
            # Reduce fox population
            num_foxes_to_kill = current_fox_population - max_fox_population
            fox_agents = [agent for agent in self._agents if isinstance(agent, Foxes) and agent.alive]

            # Randomly select foxes to kill
            foxes_to_kill = random.sample(fox_agents, num_foxes_to_kill)

            for fox in foxes_to_kill:
                fox.health = 0  # Set health to 0 to simulate death
                fox.death()  # Call death method to perform necessary actions

            print(f"Reduced fox population from {current_fox_population} to {max_fox_population}")
    ...
    def adjust_reproduction_probability(self):
        ...
        if current_rabbit_population < 20:  # Example condition, adjust as needed
        ...
        current_foxes_population = sum(1 for agent in self._agents if isinstance(agent, Foxes) and agent.alive)
        if current_foxes_population < 20:  # Example condition, adjust as needed
            Foxes.reproduction_chance = self.initial_foxes_reproduction_prob * 2  # Double reproduction probability
        else:
            Foxes.reproduction_chance = self.initial_foxes_reproduction_prob  # Reset to initial value
